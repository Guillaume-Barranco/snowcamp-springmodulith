...
public class CustomApplicationModuleDetectionStrategy implements ApplicationModuleDetectionStrategy {
    @Override
    public Stream<JavaPackage> getModuleBasePackages(final JavaPackage basePackage) {
        return basePackage.getDirectSubPackages().stream().filter(not(javaPackage -> javaPackage.getName().equals("org.snowcamp.university.springmodulith.common")));
    }
}
